                        ...
                        <div class="d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb m-0 p-0">
                                    ...
                                    <li class="breadcrumb-item text-muted active" aria-current="page">Vial List</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                                    ...
                                    <table id="default_order"class="table border table-striped table-bordered text-nowrap" style="width:100%">
                                        ...
                                        <tbody>
                                            ...
                                            if($vials) {
                                                ...
                                                <tr>
                                                    <td>
                                                        <a href="<?php echo base_url() . "vial/barcode/" . $vial['id'] . "/download"; ?>">
                                                    ...
                                                    <td>
                                                        ...
                                                        if($vial['status'] == 0){
                                                        ?>
                                                        <button type="button" class="btn waves-effect waves-light btn-sm btn-warning" style="color:white">Un-Verified</button>
                                                        <?php
                                                        } else {
                                                        ?>
                                                        <button type="button" class="btn waves-effect waves-light btn-sm btn-primary">Verified</button>
                                                        <?php
                                                        }
                                                        ?></td>
                                                </tr>
    ...
    <?php 
        if($vials) {
            foreach($vials as $vial) {
                ...
                ?>
                <div id="vaccine-modal-<?php echo $vial['id']; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h4 class="modal-title" id="myModalLabel">Vial Usage (<?php echo $vaccine['amount']; ?> Dose)</h4>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
                            </div>
                            <div class="modal-body">
                                <div class="table-responsive">
                                    <table id="default_order_<?php echo $vial['id'];?>" class="table border table-striped table-bordered text-nowrap" style="width:100%">
                                        <thead>
                                            <tr>
                                                <th>Patient Name</th>
                                                <th>Attendee</th>
                                        ...
                                        <tbody>
                                            ...
                                            <?php 
                                            $getUsageQuery = $this->db->where('vial_id', $vial['id'])->where('status', 1)->get('schedules');
                                            
                                            $usages = $getUsageQuery->result_array();

                                            if($usages) {

                                                foreach($usages as $usage) {
                                                $incident = $this->incidents->getIncident($usage['incident_id']);
                                                $patient = $this->patients->getPatient($incident['patient_id']);
                                                $user = $this->users->getUser($usage['user_id']);
                                                ?>
                                                <tr>
                                                    <td><?php echo $patient['patient_first_name']; ?> <?php echo $patient['patient_last_name']; ?></td>
                                                    <td><?php echo ucfirst($user['first_name']); ?> <?php echo ucfirst($user['last_name']); ?></td>
                                                    <td><?php echo $incident['bite_date']; ?></td>
                                                    <td><?php echo $usage['schedule']; ?></td>
                                                </tr>
                                                <?php
                                                }
                                            }
                ...
                <script>
                    $('#default_order_<?php echo $vial['id'];?>').DataTable();
                </script>
                <?php
            }
        }   
